...
GameSchema.statics.getGameForPlayer = function (gameMode, styleMode, callback) {
  ...
  if (error || !game) { //musíme tedy vytvořit
    ...
    Config.getAndIncreaseNextGamesIndex(function (error, newGameIndex) {
     // console.log("got newGameIndex " + newGameIndex);
      if (error || !newGameIndex) {
        ...
        //  console.log(sn);
          var newGame = new Game({ _id: new mongoose.mongo.ObjectId(), index: newGameIndex, shortname : sn, isRunning: false, players: 1, gameMode: gameMode, styleMode : styleMode, gameCreationTime : Date.now(), gameStartTime : -1 }); 
   ...
   //   console.log("point C");
      Game.updateOne({index : game.index}, { "$set": { "players" : (game.players + 1) } }, function (error, updated) {
     //   console.log(updated);
        if (error || !updated) {
      //    console.log("Point D");
